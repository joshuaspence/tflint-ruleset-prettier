...
func TestEosNamingRule(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			`),
			Expected: helper.Issues{
				{Rule: NewNamingRule(), Message: "Names should be snake_case (CamelCase)."},
				{Rule: NewNamingRule(), Message: "Names should be snake_case (kebab-case)."},
				{Rule: NewNamingRule(), Message: "Names should be snake_case (with.dots)."},
			},
		},
		{
			Name: "type echo",
			...
			`),
			Expected: helper.Issues{
				{Rule: NewNamingRule(), Message: eosMakeTypeEchoMessage("variable", "variable_echo")},
				{Rule: NewNamingRule(), Message: eosMakeTypeEchoMessage("local", "local_echo")},
				{Rule: NewNamingRule(), Message: eosMakeTypeEchoMessage("aws_caller_identity", "caller_echo")},
				{Rule: NewNamingRule(), Message: eosMakeTypeEchoMessage("output", "output_echo")},
			},
		},
		{
			...
			`),
			Expected: helper.Issues{
				{Rule: NewNamingRule(), Message: eosMakeTypeEchoSynonymMessage("aws_s3_bucket", "pail", "my_pail")},
			},
		},
	}
