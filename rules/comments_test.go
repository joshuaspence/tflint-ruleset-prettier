...
func TestEosCommentsRule(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			`),
			Expected: helper.Issues{
				{Rule: NewCommentsRule(), Message: eosAvoidEOLCommentsMessage},
				{Rule: NewCommentsRule(), Message: eosAvoidEOLCommentsMessage},
				{Rule: NewCommentsRule(), Message: eosAvoidEOLCommentsMessage},
				{Rule: NewCommentsRule(), Message: eosAvoidEOLCommentsMessage},
			},
		},
		{
			...
			`),
			Expected: helper.Issues{
				{Rule: NewCommentsRule(), Message: "Avoid jammed comment ('#Jamm ...')."},
				{Rule: NewCommentsRule(), Message: "Avoid jammed comment ('##Jam ...')."},
				{Rule: NewCommentsRule(), Message: "Avoid jammed comment ('//Jam ...')."},
				{Rule: NewCommentsRule(), Message: "Avoid jammed comment ('///Ja ...')."},
			},
		},
		{
			Name: "length",
			...
			`),
			Expected: helper.Issues{
				{Rule: NewCommentsRule(), Message: "Wrap comment at column 80 (currently 126)."},
				{Rule: NewCommentsRule(), Message: "Wrap comment at column 80 (currently 106)."},
			},
		},
		{
