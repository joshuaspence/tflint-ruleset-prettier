...
// ListAlphabeticalOrderRule flags list literals whose string elements are
...
func NewListAlphabeticalOrderRule() *ListAlphabeticalOrderRule {
	return &ListAlphabeticalOrderRule{
		BaseRule: BaseRule{ruleName: "list_alphabetical_order"},
	}
}
...
func (r *ListAlphabeticalOrderRule) Check(runner tflint.Runner) error {
	cfg := struct {
...
// caught too.
func (r *ListAlphabeticalOrderRule) checkBody(runner tflint.Runner, body *hclsyntax.Body, wanted map[string]struct{}, filename string, file *hcl.File, commentCache map[string][]hcl.Range) error {
...
func (r *ListAlphabeticalOrderRule) checkAttribute(runner tflint.Runner, attr *hclsyntax.Attribute, filename string, file *hcl.File, commentCache map[string][]hcl.Range) error {
...
func (r *ListAlphabeticalOrderRule) comparisonKey(s string) string {
...
func (r *ListAlphabeticalOrderRule) isSorted(keys []string) bool {
...
func (r *ListAlphabeticalOrderRule) canFix(tuple *hclsyntax.TupleConsExpr, filename string, file *hcl.File, commentCache map[string][]hcl.Range) bool {
	rng := tuple.Range()
...
func (r *ListAlphabeticalOrderRule) fileComments(filename string, file *hcl.File, cache map[string][]hcl.Range) []hcl.Range {
...
func (r *ListAlphabeticalOrderRule) buildFixed(tuple *hclsyntax.TupleConsExpr, keys []string, file *hcl.File) string {
	type element struct {
		key string
		src string
	}
